...
let desc  = `**▸ Official Rank:** #${utils.CommaFormat(profile.pp.rank)} (${profile.country}#${profile.pp.countryRank})\n`
    desc += `**▸ Level:** ${parseInt(profile.level)} (${level}%)\n`
    desc += `**▸ Total PP:** ${utils.CommaFormat(pp)}\n`
    desc += `**▸ Hit Accuracy:** ${acc}%\n`
    desc += `**▸ Playcount:** ${utils.CommaFormat(profile.counts.plays)}`
