...
async function Normal(args) {
    ...
    let fcppDisplay = ""
    //TODO: bellow
    if (mostRecent.maxCombo < beatmap.maxCombo - 15 || mostRecent.counts.miss > 0) fcppDisplay = `(${utils.RoundFixed(calculator.GetFcPP(mostRecent, beatmap))}pp for ${utils.RoundFixed(calculator.GetFcAcc(mostRecent) * 100)}% FC) `
    let description = 
`▸ ${emotes.GetEmote(mostRecent.rank)} ▸ **${utils.RoundFixed(mostRecent.pp < 0.01 ? await calculator.GetPlayPP(mostRecent, beatmap) : mostRecent.pp)}pp** ${fcppDisplay}▸ ${utils.RoundFixed(utils.CtbAccuracy(mostRecent.counts) * 100)}%
...
▸ ${emotes.GetEmote(score.rank)} ▸ **${utils.RoundFixed(score.pp)}pp** ${fcppDisplay}▸ ${utils.RoundFixed(utils.CtbAccuracy(score.counts) * 100)}%
▸ ${score.score} ▸ x${score.maxCombo}/${map.maxCombo} ▸ [${score.counts[300]}/${score.counts[100]}/${score.counts[50]}/${score.counts.miss}]
