...
async function Normal(args) {
    ...
    let beatmap = await utils.GetMap(mostRecent.beatmapId, args.Flags.m, mostRecent.raw_mods)
    ...
    let fcppDisplay = ""
    if (mostRecent.maxCombo < beatmap.maxCombo - 15 || mostRecent.counts.miss > 0)
        fcppDisplay = `(${utils.RoundFixed(await calculator.GetFcPP(mostRecent))}pp for ${utils.RoundFixed(calculator.GetFcAcc(mostRecent) * 100)}% FC) `
    
let desc  = `▸ ${emotes.GetEmote(mostRecent.rank)} ▸ **${utils.RoundFixed(mostRecent.pp < 0.01 ? await calculator.GetPlayPP(mostRecent) : mostRecent.pp)}pp** ${fcppDisplay}▸ ${utils.GetAccuracy(mostRecent.counts, args.Flags.m)}%\n`
    desc += `▸ ${utils.CommaFormat(mostRecent.score)} ▸ x${mostRecent.maxCombo}/${beatmap.maxCombo} ▸ [${mostRecent.counts[300]}/${mostRecent.counts[100]}/${mostRecent.counts[50]}/${mostRecent.counts.miss}]`
    ...
    if (beatmap.objects.normal + beatmap.objects.slider + beatmap.objects.spinner !== mostRecent.counts[300] + mostRecent.counts[100] + mostRecent.counts[50] + mostRecent.counts.miss)
        desc += `\n▸ **Map Completion:** ${utils.GetProgress(beatmap.objects, mostRecent.counts, args.Flags.m)}%`
...
let desc  = `**${score.index}. [${map.title} [${map.version}]](${utils.GetMapLink(map.id)}) +${utils.ModsFromRaw(score.raw_mods)}** [${utils.RoundFixed(map.difficulty.rating)}★]\n`
    desc += `▸ ${emotes.GetEmote(score.rank)} ▸ **${utils.RoundFixed(score.pp)}pp** ${fcppDisplay}▸ ${utils.GetAccuracy(score.counts)}%\n`
    desc += `▸ ${utils.CommaFormat(score.score)} ▸ x${score.maxCombo}/${map.maxCombo} ▸ ${utils.GetHitsDisplay(map.counts, args.Flags.m)}\n`
...
let desc  = `**${score.index}. [${map.title} [${map.version}]](${utils.GetMapLink(map.id)}) +${utils.ModsFromRaw(score.raw_mods)}** [${utils.RoundFixed(map.difficulty.rating)}★]\n`
    desc += `▸ ${emotes.GetEmote(score.rank)} ▸ **${utils.RoundFixed(score.pp)}pp** ${fcppDisplay}▸ ${utils.GetAcc(score.counts, args.Flags.m)}%\n`
    desc += `▸ ${utils.CommaFormat(score.score)} ▸ ${utils.GetCombo(score.maxCombo, map.maxCombo, args.Flags.m)} ▸ ${utils.GetHitsDisplay(map.counts, args.Flags.m)}\n`
    desc += `▸ Score Set ${utils.DateDiff(new moment(score.date), new moment(Date.now()))}Ago`
